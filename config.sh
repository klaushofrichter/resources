...
# or services that are optional, like Goldilock.
#
...
FLUENTBITCHART="0.19.16"           # see https://artifacthub.io/packages/helm/fluent/fluent-bit
KUBEPROMETHEUSSTACKCHART="27.0.1"  # see https://artifacthub.io/packages/helm/prometheus-community/kube-prometheus-stack
PROMOPERATOR="0.53.1"              # this is for Prometheus' CRDs. The version must fit to the chart version above. Checkout the 
                                   # release notes of the kube-prometheus-stack chart. For example v24->v25 required an update of
				   # the Prom Operator as well.
				   # Also note that the CRDs are loaded by "./start.sh" and not by "./prom-deploy.sh". So
				   # "./start.sh" needs to be called when the PROMOPERATOR CRD version changes.
GOLDILOCKSCHART="5.1.0"            # see https://artifacthub.io/packages/helm/fairwinds-stable/goldilocks
